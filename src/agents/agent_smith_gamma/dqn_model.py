...
class NoisyLinear(nn.Module):
  ...
  def __init__(self, in_features, out_features, std_init=0.5):
    ...
    self.weight_mu = nn.Parameter(torch.empty(size=(out_features, in_features)))
...
class DQN(nn.Module):
  def __init__(self,
    ...
    self.fc_z_v = NoisyLinear(
      hidden_size, self.atoms, std_init=noisy_std)
    self.fc_z_a = NoisyLinear(
      hidden_size, action_space * self.atoms, std_init=noisy_std)
