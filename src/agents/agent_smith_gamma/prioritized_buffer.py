...
class ReplayMemory:
  ...
  def _get_sample_from_segment(self, segment, i):
    ...
    state_lst = [trans.state for trans in transition[:self.history]]
    state = torch.stack(state_lst).to(device=self.device).to(dtype=torch.float32).div_(255)
    next_state_lst = [trans.state for trans in transition[self.n:self.n + self.history]]
    next_state = torch.stack(next_state_lst).to(device=self.device).to(dtype=torch.float32).div_(255)
